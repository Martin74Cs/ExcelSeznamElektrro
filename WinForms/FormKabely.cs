...
namespace WinForms
{
    public partial class FormKabely : Form
    {
        ...
        private void FormKabely_Load(object sender, EventArgs e)
        {
            ...
            // Inicializace filtrů
            comboBoxFilterExist.Items.Clear();
            comboBoxFilterExist.Items.AddRange(new object[] { "Vše", "Ano", "Ne" });
            comboBoxFilterExist.SelectedIndex = 0;
        ...
        private void AplikujFiltryZarizeni()
        {
            ...
            if (comboBoxFilterExist.SelectedIndex == 1) // Ano
            {
                query = query.Where(z => z.IsExist);
            }
            else if (comboBoxFilterExist.SelectedIndex == 2) // Ne
            {
                query = query.Where(z => !z.IsExist);
            }
